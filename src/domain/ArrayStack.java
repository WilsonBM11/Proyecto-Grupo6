...
public class ArrayStack implements Stack {
    ...
    public int top; //para llevar el control del tope de la pila
    private Object stack[];

    //Constructor
    public ArrayStack(int n) {
        if (n <= 0) {
            System.exit(1); //se sale
        }
        this.n = n;
    ...
    public void wrapHalf() throws StackException, QueueException {
        if (isEmpty()){
            throw new StackException("Array Stack is empty");
        }
        int half = size()/2;
        ArrayStack auxRight = new ArrayStack(half+1);
        ArrayStack auxLeft = new ArrayStack(half+1);
        while(!isEmpty()){
            if(half > 0){
                auxLeft.push(pop());
                half--;
        ...
        while(!auxLeft.isEmpty()){
            push(auxLeft.pop());
        }
        while(!auxRight.isEmpty()){
            push(auxRight.pop());
        }
